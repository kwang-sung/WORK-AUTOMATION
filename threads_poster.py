...
아래 조사된 정보를 바탕으로 스레드에 올릴 글을 자연스럽게 써주세요.
...
- 마지막 줄: "쿠대/ALL8 단체방에서 매주 수·일 무료교육 해요 → open.kakao.com/o/gKWnrBDg" 정도로 자연스럽게 (링크는 반드시 그대로)
...
def main():
    ...
    if not is_valid_post(post_text):
        ...
        send_telegram(f"🚨 스레드 발행 차단\n\nGemini 검색 결과 부족으로 비정상 응답 감지\n발행을 차단했습니다.\n\n📝 차단된 내용:\n{post_text[:300]}")
    ...
    if THREADS_ACCESS_TOKEN and THREADS_USER_ID:
        ...
        if success:
            send_telegram(f"✅ 스레드 발행 완료\n\n📝 발행된 내용:\n{post_text}")
            send_notification(post_text, "success")
        else:
            send_telegram(f"❌ 스레드 발행 실패\nThreads API 오류가 발생했습니다.\n\n📝 발행 시도 내용:\n{post_text[:300]}")
            send_notification(post_text, "blocked", "Threads API 발행 실패")
    else:
