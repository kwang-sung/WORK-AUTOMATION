...
# ─── 구매대행 정의 가드 (프롬프트 공통 주입) ──────────────
# 국내 위탁판매/역직구와 혼동해 오너클랜·도매매 같은 국내 도매몰을
# 소싱처로 제시하거나, K-뷰티를 해외에서 소싱하라는 역방향 오류를 막는다.
SOURCING_GUARD = """
## ⚠️ 구매대행 정의 (절대 위반 금지)
구매대행 = **해외에서 상품을 사서 국내 마켓에 파는 사업**. 방향은 반드시 [해외 → 국내].
국내 상품을 해외에 파는 것은 역직구이며 이 글의 주제가 아닙니다.

### 🚫 소싱처 사이트명은 절대 쓰지 마세요 (가장 중요)
특정 쇼핑몰·플랫폼 이름을 소싱처로 지목하는 것은 **전면 금지**입니다.
그 사이트가 실제로 해당 상품을 취급하는지 검증할 수 없어, 틀리면 글 전체의 신뢰가 무너집니다.

❌ 금지 예시: "소싱처: 1688 코스메틱관, 오너클랜, 도매매, 타오바오 OO관"
❌ "OO몰에서 찾을 수 있습니다", "OO닷컴 검색 추천" 같은 우회 표현도 동일하게 금지

✅ 대신 이렇게 쓰세요 — **소싱 지역과 유형까지만**:
   "중국 도매 플랫폼 / 소형 세트 구성 위주"
   "일본 종합몰 / 한정판·굿즈 카테고리"
   "미국·유럽 아울렛 / 시즌오프 재고"

❌ 국내 도매몰(오너클랜, 도매매, 도매꾹, 온채널, KSeller, 셀러오션 등)은
   국내 B2B 위탁 플랫폼이라 구매대행과 무관하므로 언급 자체를 금지합니다.

소싱 국가는 중국·일본·미국·유럽·동남아 등 **해외**만 해당합니다.

### 아이템 선정 금지
- 한국이 원산지·강점인 카테고리(K-뷰티, K-푸드, 국산 화장품·식품·패션)를
  "해외에서 소싱하라"고 제안하는 것 → 방향이 거꾸로라 명백한 오류입니다.
- 반드시 **국내에 없거나 비싼 해외 상품**을 소싱 대상으로 제시하세요.
"""


# ─── 셀러 체크포인트 (Gemini 검색 기반) ──────────────────
# 소스 매체를 명시해 일반 검색보다 업계 심층 기사·보도자료가 잡히게 유도
CHECKPOINT_QUERIES = [
    ("플랫폼·정책",
     "바이라인네트워크 OR 아웃스탠딩 OR 전자신문 이커머스 플랫폼 정책 변화 셀러 영향 기사"),
    ("유통 실적·데이터",
     "뉴스와이어 보도자료 OR 유통업계 발표 이커머스 판매 실적 거래액 증가율 데이터"),
    ("해외·역직구",
     "더구루 OR 어패럴뉴스 OR 플래텀 역직구 해외 진출 중국 이커머스 채널 최신"),
    ("소비 트렌드",
     "Z세대 소비 트렌드 리셀 한정판 굿즈 품절 대란 판매 데이터 국내"),
    ("통관·규제",
     "관세청 수입통관 규정 변경 해외직구 구매대행 KC인증 최신"),
]
...
def build_seller_checkpoint_section(history: dict | None = None) -> str:
    ...
    for label, query in CHECKPOINT_QUERIES:
        try:
            prompt = (
                f"[{date_ctx}] {query} — 셀러에게 실질적으로 중요한 최신 정보 3가지를 찾아 "
                f"'제목 | 핵심내용 2~3문장' 형식으로 한국어 답변.\n"
                f"⚠️ 반드시 구체적 수치(증가율%, 금액, 거래량, 판매건수, 기간)를 포함할 것. "
                f"수치 없는 추상적 내용은 제외. 확인된 수치만 쓰고 추정이면 (추정) 표기."
                f"{exclude_str}"
            )
    ...
    prompt = f"""아래는 이번 주 업계 매체·보도자료에서 수집한 이커머스·구매대행 최신 정보입니다.
셀러에게 실질적으로 유용한 핵심 체크포인트 3~4가지를 뽑아 HTML로 작성하세요.

## 각 항목 구조 (반드시 이 3단 구성)
1) 제목 — 한 줄, 궁금증을 자극하되 과장 금지
2) 사실 본문 — 3~4문장. 수집 자료의 구체적 수치(증가율%·금액·거래량·기간)를 반드시 1개 이상 인용.
   숫자 없이 "늘고 있다", "주목받고 있다" 식의 뭉뚱그린 서술 금지.
3) 셀러 액션 — 2~3문장. "그래서 셀러는 뭘 해야 하나"를 구체적 행동으로.
   ✅ 좋은 예: "낱개 피규어를 소싱해 직접 세트로 묶어 파는 방식을 시도해보세요."
   ❌ 나쁜 예: "시장 동향을 주시할 필요가 있습니다." (행동이 아님)

## 금지
- 외부 링크, 출처·매체명 표기 (쿠대 마스터 본인의 분석처럼 서술)
- 수집 자료에 없는 수치 창작
- 액션 문단을 본문 요약으로 때우는 것
- **소싱처 사이트명 지목** (1688, 타오바오, 오너클랜, 도매매 등 일체 금지)
  → 검증 불가라 틀리면 신뢰를 잃습니다. "중국 도매 플랫폼" 수준의 지역·유형까지만.

## HTML 규칙
- 네이버 카페 호환: div background-color 금지, table 구조 사용, flex/grid 금지
- 섹션 헤더 포함: "📋 이번 주 셀러 체크포인트"
- 셀러 액션 문단은 시각적으로 구분 (예: ✅ 아이콘 + 진한 글씨)
...
독자는 **해외 상품을 소싱해 국내 마켓(쿠팡·스마트스토어 등)에 파는 구매대행 셀러**입니다.
{SOURCING_GUARD}
...
3. 📌 이번 주 소싱 기회 — "뭘 팔까?" [전술|소싱아이템]
   - 트렌딩 아이템 3~4개
   - 아이템별 구성 (이 순서 고정):
     · 왜 지금 팔리는지 3~4문장 (가능하면 수집 정보의 수치 인용)
     · 소싱 방향: 지역 + 상품 유형까지만 (예: "중국 도매 / 소형 세트 구성")
       ⚠️ 사이트명·쇼핑몰명 절대 금지
     · 예상 마진율 (범위로, 근거 한 줄 포함)
     · 소싱 전 확인사항: 인증·유통기한·라이선스·통관 등 실제로 걸릴 수 있는 것 1~2개
   - 실전 소싱 팁 박스
